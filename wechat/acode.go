...
func (wx *Wechat) GetWxAcodeUnlimit(scene, page string, appId, appSecret string) ([]byte, error) {
	...
	if err != nil {
		log.Errorf(nil, "cannot get access token: %v", err.Error())
		return nil, err
	}
	...
	log.Infof(nil, "get wx acode unlimit: %v %v", requestURL.String(), string(jData))
	...
	if err != nil {
		log.Errorf(nil, "get wx acode unlimit err: %v %v %v", err, requestURL.String(), string(jData))
	}
	return respData, err
}
